...
HANDLED_NORMALIZATION = [
    'min_max_scaling', 'individual_min_max_scaling', None]
...
class DataProcessor:
    ...
    **minimal shape**. 
    ...
    def __init__(self, data_normalization_type='min_max_scaling'):
        ...
        if not data_normalization_type in HANDLED_NORMALIZATION:
            raise RuntimeError("Wrong normalization type provided. "
                f"Handled: {HANDLED_NORMALIZATION}. Check doc for further details.")

        self.data_normalization = data_normalization_type

    def process_data(
        self, data: Union[np.ndarray, torch.Tensor, List[Union[np.ndarray, torch.Tensor]]]
        ) -> torch.tensor:
        ...
        if isinstance(data, list):
            for d in data:
                if not isinstance(d, np.ndarray) and not torch.is_tensor(d):
                    raise TypeError("Unhandled type in provided data. Expect list of 'np.ndarray' "
            ...
            raise TypeError("Wrong data type provided. Expected one of "
    ...
    def to_dataset(data: torch.Tensor, labels: Optional[torch.Tensor]=None):
        """This method converts a set of ``torch.Tensor`` to a :class:`~pyraug.data.datasets.BaseDataset`
    ...
    def _individual_min_max_scaling(data: torch.Tensor) -> torch.Tensor:
        ...
        clean_data = (
            d - d.min(dim=-1).values.unsqueeze(-1)
        ) / (
            d.max(dim=-1).values.unsqueeze(-1)
            - d.min(dim=-1).values.unsqueeze(-1)
        )
    ...
    def _min_max_scaling(data: torch.Tensor)  -> torch.Tensor:
        ...
        clean_data = (
            data - data.min().unsqueeze(-1)
        ) / (
            data.max().unsqueeze(-1)
            - data.min().unsqueeze(-1)
        )
    ...
    def _process_data_list(
        self, data_list: List[
            Union[torch.Tensor, np.ndarray]]) -> torch.tensor:
        ...
        for data in data_list:
            ...
            if min_shape == ():
                ...
                if len(data.shape) != len(min_shape):
                    raise RuntimeError(f"Found {len(data.shape)}D and {len(min_shape)}D images ! "
        ...
        if diff_shape_count >= 1:
            ...
            logger.info(f" -> Reshaped data to minimum size (data of shape: {tuple(data.shape)}).\n")
    ...
    @staticmethod
    def reshape_data(data_list: List[torch.Tensor], target_shape: Tuple[int, ...]) -> torch.Tensor:
    ...
    @staticmethod
    def _reshape_data(data: torch.Tensor, target_shape: Tuple[int, int]) -> torch.Tensor:
        """
        This function takes an input data and reshape it to a target shape
        ...
        if len(data.shape) == 3:
            ...
            raise NotImplementedError("Reshaping only implemented for 3D, 4D and 5D data")
