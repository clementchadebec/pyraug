...
class Test_DataLoader:
    ...
    def test_build_eval_data_loader(self, model_sample, train_dataset, training_config_batch_size):
        trainer = Trainer(
            model=model_sample,
            train_dataset=train_dataset,
            training_config=training_config_batch_size
...
class Test_Set_Training_config:
    ...
    def test_set_training_config(self, model_sample, train_dataset, training_configs):
        ...
        if training_configs is None:
            assert trainer.training_config == TrainingConfig(output_dir='dummy_output_dir')
...
class Test_Build_Optimizer:
    ...
    def optimizers(self, request, model_sample, training_configs_learning_rate):
        ...
        optimizer = request.param(model_sample.parameters(), lr=training_configs_learning_rate.learning_rate)
        return optimizer
    ...
    def test_default_optimizer_building(
        ...
        assert trainer.optimizer.defaults['lr'] == training_configs_learning_rate.learning_rate

    def test_set_custom_optimizer(self, model_sample, train_dataset, training_configs_learning_rate, optimizers):
        trainer = Trainer(
            model=model_sample,
            train_dataset=train_dataset,
            training_config=training_configs_learning_rate,
...
class Test_Init_EarlyStopping_Flags:
    ...
    @pytest.fixture(params=[
        ...
        (
            TrainingConfig(
                train_early_stopping = 50,
                eval_early_stopping = 50
            ),
            (False, True)
        ),
        (
            TrainingConfig(
                train_early_stopping = None,
                eval_early_stopping = 50
            ),
            (False, True)
        ),
...
class Test_RHVAE_Training:
    ...
    def test_rhvae_train_step(self, rhvae_sample, train_dataset, training_configs, optimizers):
        trainer = Trainer(
            model=rhvae_sample,
            train_dataset=train_dataset,
            training_config=training_configs,
        ...
        assert not all([
            torch.equal(
                start_model_state_dict[key],
                step_1_model_state_dict[key]
                ) for key in start_model_state_dict.keys()
        ])


    def test_rhvae_eval_step(self, rhvae_sample, train_dataset, training_configs, optimizers):
        trainer = Trainer(
            model=rhvae_sample,
            train_dataset=train_dataset,
            eval_dataset=train_dataset,
    ...
    def test_rhvae_main_train_loop(self, tmpdir, rhvae_sample, train_dataset, training_configs, optimizers):
...
class Test_RHVAE_Saving:
    ...
    def test_checkpoint_saving(self, tmpdir, rhvae_sample, train_dataset, training_configs, optimizers):
...
class Test_Logging:
    ...
    def test_create_log_file(self, tmpdir, model_sample, train_dataset, training_config):
