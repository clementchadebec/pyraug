...
class Test_Pipeline:
    ...
    def test_pipeline(self, tmpdir, rhvae_config, rhvae_sampler_config, samples_number):
        ...
        assert all([
            f"generated_data_{rhvae_sampler_config.samples_per_save}_{i}.pt" in files_list
            for i in range(full_data_file_nbr)
        ])

        if last_file_data_nbr > 0:
            assert  f"generated_data_{last_file_data_nbr}_{expected_num_of_data_files-1}.pt" in files_list
        ...
        for i in range(full_data_file_nbr):
            data_rec.append(
                torch.load(
                    os.path.join(
                        data_folder, "generated_data_"
                        f"{rhvae_sampler_config.samples_per_save}_{i}.pt")))

        if last_file_data_nbr > 0:
            data_rec.append(torch.load(
                    os.path.join(
                        data_folder, f"generated_data_"
